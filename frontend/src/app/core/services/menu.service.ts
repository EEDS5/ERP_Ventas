...
export class MenuService {
  ...
  /** Deep clone object that can be JSON stringified */
  private deepClone<T>(obj: T): T {
    return JSON.parse(JSON.stringify(obj));
  }

  /** Whether two JSON-serializable objects are equal */
  private isJsonObjEqual<T>(obj0: T, obj1: T): boolean {
  ...
  /** Get the menu level path from the route segments */
  getLevel(routeArr: string[]): string[] {
    ...
    this.menu$.value.forEach((item) => {
      ...
      while (unhandledLayer.length > 0) {
        const nextUnhandledLayer: MenuTraversalNode[] = [];
